define(['entities/domain/JsonAbstractEntity'], /**@lends JsonAbstractCollection*/
     ...
     * @param {int} width the width of the svg-element
     * @param {int} height the height of the svg-element
     * @param {string} color the color
     * @constructor
     */
    function JsonAbstractCollection(id, name, type, x, y, width, height, color){
        JsonAbstractEntity.call(this, id, name, type, x, y, width, height, color);
    ...
    JsonAbstractCollection.prototype.add = function(entity){
        if(!entity) {
            console.error('json entity is null');
            return false;
        }
        ...
        if (key && !this.getElements().hasOwnProperty(key)) {
            ...
            this.get$node().children('.properties').append(entity.get$node());
            return true;
        }
